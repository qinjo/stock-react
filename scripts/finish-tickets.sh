...
gh issue close 6 --comment "T5 完成（908f7dc、64a0c81）：分析缓存（键=股票+交易日，TTL 可配、abstain 不入缓存；原全文哈希方案因盘中 tick 导致永不命中，已修正）+ token 用量透传至前端+ 信号卡 + 五节报告 + 三态异常 + 免责声明。前端 32 项测试覆盖三态分流。真实 LLM 端到端待 key 验证。"
